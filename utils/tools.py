...
def compute_distances_cp_chunked(
    ...
    for i in range(0, number_of_words, chunk_size):
        ...
        distances[i:j, :] = (
            distance.cdist(words_embeddings[i:j], vocab_cp, metric).astype(dtype).get()
        )
...
def compute_distances(
    ...
    return compute_distances_cp_chunked(
        words_embeddings, vocabulary, metric, dtype=dtype
    )
...
def argsort_chunked(
    array: np.ndarray, dtype: np.dtype, chunk_size: int = 500
) -> np.ndarray:
    ...
    then require a copy when .astype() is called, this function performs the
    argsort chunk by chunk."""
